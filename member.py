...
class Member:
    ...
    def get_all_members():
        members, _ = read_data()
        print("Members data read from database:", len(members))  # Debug print
        active_members = []
        for member in members:
            exit_date_str = member.get('exit_date')
            if exit_date_str:
                if isinstance(exit_date_str, date):
                    exit_date_obj = exit_date_str
                else:
                    try:
                        exit_date_obj = datetime.strptime(exit_date_str, '%Y-%m-%d').date()
                    except ValueError as e:
                        print(f"Error parsing exit_date for member {member['member_id']}: {e}")
                        continue
                if exit_date_obj == date(9999, 12, 31):
                    active_members.append(member)
        print("Active Members:", len(active_members))
    ...
    def generate_member_id():
        ...
        print([member.get('member_id', '0') for member in members])
        existing_ids = [
    ...
    def add_member(member_data, identity_document=None, photo_id=None, upload_folder=None):
        ...
        print("Member Room Details", room_number)
        if room_number:
            # Find the room and update 'Occupied Beds'
            for room in rooms:
                print("Room Details", room)
                print("Room Number of room", room['room_number'])
                if str(room['room_number']) == str(room_number):
                    ...
                    room['occupied_beds'] += 1  # Increment 'Occupied Beds'
                    break
            else:
        ...
        try:
            for member in members:
                if member.get('member_id') == member_id:
                    ...
                    Room.update_occupied_beds(member['room_number'])  # Recalculate occupied beds
                    break
            else:
    ...
    def search_member(search_type, search_value):
        ...
        for member in members:
            ...
            elif search_type == 'room' and search_value == member['room_number']:
                search_results.append(member)
            elif search_type == 'phone' and search_value == member['phone_number']:
                search_results.append(member)
